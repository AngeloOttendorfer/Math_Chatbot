...
def run_eval(args, trial_id):
    ...
    if args.arch in {'gpt2'}:
        print(
            f"Loading model from <path to the model>")
        ...
        print("Successfully loaded model from <path to the model>")
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(tqdm(dataloader)):
            ...
            batch_ids = batch['input_ids'][0].unsqueeze(0)
            reprompt_ids = torch.LongTensor(tokenizer.encode(
                f"You have made a calculation error. The correct answer should be {correct_ans}. Please try again, ensuring that all steps lead to the correct solution.",
                verbose=False, return_tensors="pt"))
            print("reprompt_ids: " + str(reprompt_ids))
            ...
            for j in range(num_samples):
                if j == 0:
                    print("Using batch['input_ids']")
                else:
                    batch['input_ids'] = batch_reprompt
                    print("Using batch_reprompt")
                ...
                if j == 0:
                    ...
                    output_tokens = get_model_output(batch_reprompt[0], output_ids[0], tokenizer)

                generated_ans = tokenizer.decode(output_tokens)
                if is_equiv(generated_ans, correct_ans):
                    break

                batch_reprompt = torch.cat((
                    reprompt_ids,
                    batch_ids,
                ), dim=1)
            ...
            outputs.append(generated_ans)
            answers.append(correct_ans)

            equiv = is_equiv(generated_ans, correct_ans)
            if equiv:
                correct += 1
    ...
    output_file = os.path.join(args.output_dir, f"Outputs_400_1_v3_{trial_id}.txt")
